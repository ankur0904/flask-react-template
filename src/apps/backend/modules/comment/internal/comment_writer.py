...
class CommentWriter:
    ...
    def delete_comment(*, params: DeleteCommentParams) -> CommentDeletionResult:
        comment = CommentReader.get_comment(
            params=GetCommentParams(task_id=params.task_id, comment_id=params.comment_id, account_id=params.account_id)
        )
        ...
        return CommentDeletionResult(comment_id=comment.id, deleted_at=deletion_time, success=True)
